...
// Opening this file by double-click (file://) silently breaks ES module
// imports, Google sign-in, and every Firebase call. Warn loudly instead of
// leaving the person staring at a page where nothing responds.
if (location.protocol === "file:") {
  const w = document.getElementById("fileProtocolWarning");
  if (w) w.classList.remove("hidden");
  console.error("GameSouq: running from file:// — serve this folder over http(s) instead (see README.md).");
}
window.addEventListener("error", (e) => console.error("GameSouq error:", e?.error || e?.message || e));
...
// --------------------------------------------------------------------------
// ICONS — hand-authored inline SVGs (no external icon font/CDN dependency)
// --------------------------------------------------------------------------
const ICON_PATHS = {
  shield: `<path d="M12 3l7 3v6c0 4.5-3 7.5-7 9-4-1.5-7-4.5-7-9V6l7-3z"/>`,
  "shield-check": `<path d="M12 3l7 3v6c0 4.5-3 7.5-7 9-4-1.5-7-4.5-7-9V6l7-3z"/><path d="M9 12l2 2 4-4"/>`,
  crown: `<path d="M3 8l4 3 5-6 5 6 4-3-2 10H5L3 8z"/><path d="M5 18h14"/>`,
  lock: `<rect x="5" y="11" width="14" height="9" rx="1.5"/><path d="M8 11V8a4 4 0 0 1 8 0v3"/>`,
  "check-circle": `<circle cx="12" cy="12" r="9"/><path d="M8 12.5l2.5 2.5L16 9.5"/>`,
  gamepad: `<rect x="3" y="8" width="18" height="9" rx="4"/><path d="M8 11v4M6 13h4"/><circle cx="16" cy="12" r="1" fill="currentColor" stroke="none"/><circle cx="18" cy="14" r="1" fill="currentColor" stroke="none"/>`,
  circle: `<circle cx="12" cy="12" r="6" fill="currentColor" stroke="none"/>`,
};

function icon(name, cls = "w-4 h-4 inline-block align-[-2px]") {
  return `<svg viewBox="0 0 24 24" class="${cls}" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round">${ICON_PATHS[name] || ""}</svg>`;
}

function starIcon(filled) {
  return `<svg viewBox="0 0 24 24" class="w-4 h-4 inline-block ${filled ? "text-gold-400" : "text-dim"}" fill="${filled ? "currentColor" : "none"}" stroke="currentColor" stroke-width="1.4" stroke-linecap="round" stroke-linejoin="round"><path d="M12 2.5l2.9 6.6 7.1.6-5.4 4.7 1.6 7-6.2-3.8-6.2 3.8 1.6-7-5.4-4.7 7.1-.6z"/></svg>`;
}
...
function starsHTML(avg, count) {
  ...
  if (typeof count === "number") {
    out += `<span class="text-xs text-dim mr-1">${count ? `(${(avg || 0).toFixed(1)} · ${count})` : "لا يوجد تقييمات بعد"}</span>`;
  }
  return out;
}

// badge shown next to a name when that user is an admin or a verified middleman
function roleBadgeInline(role, nameColor) {
  if (role === "admin") {
    return `<span class="inline-flex items-center gap-1 font-bold" style="color:${esc(nameColor || "#E8C574")}">${icon("crown")}المشرف</span>`;
  }
  if (role === "middleman") {
    return `<span class="inline-flex items-center gap-1 font-bold" style="color:${esc(nameColor || "#3FBFAE")}">${icon("shield-check")}وسيط موثّق</span>`;
  }
  return "";
}
...
// AUTH
// --------------------------------------------------------------------------
// prefer the profile photo/name the user set inside GameSouq over the raw
// Google account values, since editing your profile only updates Firestore.
function myPhoto() { return currentUserDoc?.photoURL || currentUser?.photoURL || ""; }
function myName() { return currentUserDoc?.name || currentUser?.displayName || "مستخدم"; }
...
function renderAuthUI() {
  ...
  if (currentUser) {
    authArea.innerHTML = `<img src="${esc(myPhoto())}" class="w-8 h-8 rounded-full border divider object-cover" />`;
    authBtn.textContent = "تسجيل الخروج";
    authBtn.onclick = () => signOut(auth);

    menuProfile.classList.remove("hidden");
    $("#menuAvatar").src = myPhoto();
...
function roleBadgeHTML(role) {
  if (role === "admin") return `<span class="text-gold-400 font-bold inline-flex items-center gap-1">${icon("crown")}المشرف الرئيسي</span>`;
  if (role === "middleman") return `<span class="text-teal-400 font-bold inline-flex items-center gap-1">${icon("shield-check")}وسيط موثّق</span>`;
...
function offerCardHTML(o) {
  ...
  const badge = roleBadgeInline(o.posterRole, o.posterNameColor);
  return `
  <article data-open-offer="${o.id}" class="stall-card rounded-xl overflow-hidden cursor-pointer">
    <div class="flex gap-3 p-3">
      <div class="w-20 h-20 rounded-lg overflow-hidden shrink-0 surface-alt flex items-center justify-center">
        ${img ? `<img src="${esc(img)}" class="w-full h-full object-cover" />` : icon("gamepad", "w-7 h-7 text-gold-500/50")}
      </div>
      <div class="min-w-0 flex-1">
        ...
        <div class="flex items-center justify-between mt-2 gap-2">
          ...
          <span class="text-[11px] text-dim inline-flex items-center gap-1 truncate">${badge}${esc(o.posterName || "")}</span>
        </div>
      </div>
    </div>
  </article>`;
...
async function openOfferModal(id, cached) {
  ...
  $("#offerModalBody").innerHTML = `
    ...
    <div class="flex items-center gap-2 pt-2 border-t divider">
      <img src="${esc(offer.posterPhoto || "")}" class="w-9 h-9 rounded-full border divider object-cover" />
      <div class="min-w-0">
        <span class="text-sm font-bold flex items-center gap-1">${roleBadgeInline(offer.posterRole, offer.posterNameColor)}${esc(offer.posterName || "")}</span>
      </div>
    </div>
    ${isOwner
...
$("#offerForm").addEventListener("submit", async (e) => {
  ...
  try {
    ...
    await addDoc(collection(db, "offers"), {
      ...
      posterName: myName(),
      posterPhoto: myPhoto(),
      posterRole: currentUserDoc?.role || "user",
      posterNameColor: currentUserDoc?.nameColor || "",
      status: "available",
      createdAt: serverTimestamp(),
    });
...
function renderChatHeader() {
  ...
  $("#chatSubtitle").innerHTML = c.middlemanId
    ? `مع ${esc(otherName)} <span class="mx-1">·</span> ${icon("shield-check", "w-3 h-3 text-teal-400")} الوسيط: ${esc(c.middlemanName)}`
    : `مع ${esc(otherName)}`;
  ...
  $("#btnRequestMM").innerHTML = c.middlemanRequested
    ? `${icon("shield", "w-3.5 h-3.5")}بانتظار وسيط...`
    : `${icon("shield", "w-3.5 h-3.5")}طلب وسيط`;
  $("#btnRequestMM").classList.add("inline-flex", "items-center", "gap-1");
  ...
  $("#btnCompleteTrade").innerHTML = `${icon("check-circle", "w-3.5 h-3.5")}إتمام الصفقة`;
  $("#btnCompleteTrade").classList.add("inline-flex", "items-center", "gap-1");
...
function renderChatMessages(msgs) {
  ...
  wrap.innerHTML = visible.map((m) => {
    ...
    const badge = roleBadgeInline(m.senderRole, m.senderNameColor);
    return `
    <div class="flex ${mine ? "justify-start" : "justify-end"}">
      <div class="max-w-[75%] rounded-2xl px-3 py-2 text-sm ${
        ...
        ${isPrivate ? `<div class="text-[10px] font-bold mb-0.5 flex items-center gap-1">${icon("lock", "w-3 h-3")}سري — للوسيط</div>` : ""}
        <div class="text-[10px] ${mine && !isPrivate ? "text-ink-900/70" : "text-dim"} mb-0.5 font-bold flex items-center gap-1">${badge}${esc(m.senderName)}</div>
        <div>${esc(m.text)}</div>
      </div>
    </div>`;
  }).join("");
...
$("#chatForm").addEventListener("submit", async (e) => {
  ...
  try {
    await addDoc(collection(db, "chats", activeChat.id, "messages"), {
      ...
      senderName: myName(),
      senderRole: currentUserDoc?.role || "user",
      senderNameColor: currentUserDoc?.nameColor || "",
      text,
...
async function loadMiddlemanRequests() {
  ...
  list.innerHTML = rows.map((c) => `
    <div class="stall-card rounded-xl p-3 flex items-center justify-between gap-3">
      ...
      <button data-join="${c.id}" class="shrink-0 text-xs px-3 py-1.5 rounded-lg bg-teal-500 text-ink-950 font-bold inline-flex items-center gap-1">${icon("shield-check", "w-3.5 h-3.5")}انضمام كوسيط</button>
    </div>
  `).join("");
...
function renderStarPicker() {
  ...
  for (let i = 1; i <= 5; i++) {
    ...
    s.type = "button";
    s.innerHTML = starIcon(i <= selectedStars).replace('class="w-4 h-4', 'class="w-8 h-8');
...
async function renderProfileView() {
  ...
  currentUserDoc = { ...currentUserDoc, ...u };
  $("#profileAvatar").src = myPhoto();
  $("#profileName").textContent = u.name || currentUser.displayName;
  $("#profileRole").innerHTML = roleBadgeHTML(u.role);
  $("#profileRating").innerHTML = starsHTML(u.ratingAvg, u.ratingCount || 0);
  ...
  $("#myChats").innerHTML = chats.length ? chats.map((c) => `
    <button data-open-chat="${c.id}" class="w-full text-right stall-card rounded-xl p-3 flex items-center justify-between gap-2">
      <div class="min-w-0">
        ...
        <div class="text-xs text-dim flex items-center gap-1">
          ${c.status === "completed"
            ? `${icon("check-circle", "w-3.5 h-3.5 text-teal-400")}مكتملة`
            : `<span class="w-2 h-2 rounded-full bg-teal-400 inline-block"></span>نشطة`}
          ${c.middlemanId ? `<span class="mx-1">·</span>${icon("shield-check", "w-3.5 h-3.5 text-gold-400")}بوسيط` : ""}
        </div>
      </div>
    </button>
...
$("#btnEditProfile").addEventListener("click", () => {
  ...
  $("#editProfilePreview").src = myPhoto();
  $("#editProfileNameInput").value = myName();
  $("#editProfileStatus").textContent = "";
  $("#editProfileModal").classList.remove("hidden");
  $("#editProfileModal").classList.add("flex");
});
$$("[data-close-editprofile]").forEach((el) => el.addEventListener("click", () => {
...
$("#btnSaveProfile").addEventListener("click", async () => {
  ...
  if (!name) { status.textContent = "الاسم مطلوب"; return; }
  btn.disabled = true;
  ...
  try {
    ...
    if (editProfilePhotoFile) {
      ...
      update.photoURL = await uploadToImgBB(editProfilePhotoFile);
    }
    await updateDoc(doc(db, "users", currentUser.uid), update);
    currentUserDoc = { ...currentUserDoc, ...update };
    renderAuthUI();
    renderProfileView();
    ...
    console.error(err);
    status.textContent = "تعذّر حفظ التغييرات";
  } finally {
    btn.disabled = false;
...
function guardAdminView() {
  ...
  if (currentUser?.email !== MAIN_ADMIN_EMAIL) {
    el.innerHTML = `<div class="p-10 text-center text-dim flex flex-col items-center gap-2">${icon("lock", "w-8 h-8")}<span>هذه الصفحة مخصصة للمشرف الرئيسي فقط.</span></div>`;
  }
}
